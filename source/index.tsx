...
const DEFAULT_FROM = `${BUSINESS_NAME}\n${DTI_LINE}\n${TIN_LINE}\nPembo\nTaguig Manila, Philippines\n+63956 533 1521`;
...
const isTruffleScentInvoice = (input: Partial<Invoice> | null | undefined): boolean => {
    if (!input) return false;
    const from = (input.from || '').toLowerCase();
    const invoiceNumber = (input.invoiceNumber || '').toLowerCase();
    return from.includes('truffle & scent') || from.includes('truffle and scent') || invoiceNumber.startsWith('inv-t&s');
};

const ensureInvoiceDefaults = (input: Partial<Invoice> | null | undefined): Invoice => {
    const source = input || {};
    const merged = {
        ...defaultInvoice,
        ...source,
    ...
    if (isTruffleScentInvoice(merged)) {
        return {
            ...merged,
            logo: DEFAULT_LOGO,
            from: DEFAULT_FROM,
    ...
    return {
        ...merged,
        logo: merged.logo || DEFAULT_LOGO,
        from: merged.from || DEFAULT_FROM,
    };
};

const readStoredInvoice = (): Invoice => {
    try {
        const savedInvoice = localStorage.getItem('currentInvoice');
        const parsedInvoice = savedInvoice ? JSON.parse(savedInvoice) : defaultInvoice;
        const migratedInvoice = ensureInvoiceDefaults(parsedInvoice);
        localStorage.setItem('currentInvoice', JSON.stringify(migratedInvoice));
        return migratedInvoice;
    } catch (error) {
        localStorage.setItem('currentInvoice', JSON.stringify(defaultInvoice));
        return defaultInvoice;
    }
};

const App = () => {
    const [invoice, setInvoice] = useState<Invoice>(() => readStoredInvoice());
    ...
    useEffect(() => {
        try {
            ...
            if (saved) {
                const migratedSavedInvoices = JSON.parse(saved).map((savedInvoice: Partial<Invoice>) => ensureInvoiceDefaults(savedInvoice));
                setSavedInvoices(migratedSavedInvoices);
                localStorage.setItem('savedInvoices', JSON.stringify(migratedSavedInvoices));
            }
        } catch (error) {
            setSavedInvoices([]);
